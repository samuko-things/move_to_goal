...
class MoveToGoal(Node):
  ...
  def __init__(self):
    ...
    super().__init__('move_to_goal_node2')
  ...
  def move_to_pose(self, x_pose, y_pose, theta_pose, angle_error_rad, dist_error_m):
    """
    ...
    while True:
      if (abs(rho) <= dist_error_m and abs(beta) <= angle_error_rad) or self.stop_cmd_is_available:
        self.v = 0.000
        self.w = 0.000
        break
      ...
      # msg = f"[rho:{round(rho,3)},  beta:{round(abs(beta),3)}]"
      # print(msg)
  ...
  def rotate_to_pose(self, theta_goal, angle_error_rad):
    """
    ...
    while True:
      if (abs(beta) <= angle_error_rad) or self.stop_cmd_is_available:
      ...
      # msg = f"[beta:{round(rad_2_deg(beta),3)}]"
      # print(msg)
  ...
  def move_to_goal(self, goal_pose_array, theta_goal):
    ...
    self.rotate_to_pose(theta_goal=theta_goal, angle_error_rad=deg_to_rad(2))
...
############### RUN IN THREAD ####################################################
  def execute_action(self):
    ...
    goal_pose_array = [(1,0), (2,0), (2,1), (2,2), (1,2), (0,2), (0,3), (0,4), (1,4), (2,4), (2,5), (2,6), (1,6), (0,6), 
